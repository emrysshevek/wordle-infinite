...
function App() {
  ...
  const loadDifficulty = (): string => {
    var diff = localStorage.getItem('difficulty')
    return diff ? diff : 'medium'
  }
  ...
  const [currStats, setCurrStats] = useState(stats.all)
  ...
  const updateStatsView = (newStats: CategoryStats) => {
  ...
  const onChar = (value: string) => {
    if (currentGuess.length < rowLength && guesses.length < 6 && !isGameWon) {
